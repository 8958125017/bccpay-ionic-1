...
mybccApp.controller('AppCtrl', function($scope, $ionicModal, $timeout, ionicMaterialInk, $http) {
  ...
  $scope.doLogin = function() {
    ...
    alert("fuck");
     $http.post(constants.apiurl + '/user/getChart',  {"lastHours": 24,
    "maxRespArrSize": 100}, {
        headers: {
        ...
        console.log("gettting data from server " + angular.toJson(response));
          $scope.chartDate= response;
        
        //console.log(response);
      });
      google.charts.load('current', {packages: ['corechart', 'line']});
      google.charts.setOnLoadCallback(drawBasic);
      $http.post(constants.apiurl + '/user/getChart',  {"lastHours": 24, "maxRespArrSize": 100}, {
            headers: {
              'Content-Type': 'application/json;charset=UTF-8'
            }
          }).then(function(response) {
            console.log("gettting data from server " + response);
               $scope.chartDate= response;
...
mybccApp.controller('BuyCryptoCtrl',function($scope) {


    
})

mybccApp.controller('SellCryptoCtrl',function($scope) {
...
mybccApp.controller('BidCryptoCtrl',function($scope) {


    
})
mybccApp.controller('AskCryptoCtrl',function($scope) {
...
mybccApp.controller('DashboardCtrl', function($rootScope, $scope, $ionicPopup, $state, $window, $timeout, $http, MyPayService, $localStorage) {

  $rootScope.user = $localStorage.credentials.user;
  console.log("email" + $rootScope.user.email);

  $rootScope.$on('user', function(event, obj) {
    console.log(obj.user.email); // 10
    $scope.emailId = {
      "userMailId": obj.user.email
    }
  })
  ...
  $state.reload();
  $scope.doRefresh = function(emailId) {
    $timeout(function() {
